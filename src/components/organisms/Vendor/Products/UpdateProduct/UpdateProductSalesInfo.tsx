...
export const UpdateProductSalesInfo = () => {
  const {
    register,
    setValue,
    watch,
  ...
  const productVariants = watch("productVariantRequests");
  ...
  useEffect(() => {
    const initializeAttributeGroups = (variants: ProductVariantRequest[]) => {
      const groups: AttributeGroup[] = [];
      const allAttributes = new Set<string>();

      variants.forEach((variant) => {
        variant.productAttributeRequests.forEach((attr) => {
      ...
      allAttributes.forEach((attrName) => {
        ...
        variants.forEach((variant) => {
          const attr = variant.productAttributeRequests.find(
            (a) => a.name === attrName
          );
          if (attr) {
    ...
    if (
      ...
      // Initialize matrix data
      const matrix: MatrixData = productVariants.map((variant) => [
        ...variant.productAttributeRequests.map((attr) => ({
          value: attr.value,
          type: "attribute" as const,
        })),
        { value: variant.originalPrice, type: "originalPrice" as const },
        { value: variant.price, type: "price" as const },
        { value: variant.quantity, type: "quantity" as const },
        { value: variant.sku, type: "sku" as const },
        { value: variant.image || "", type: "image" as const },
      ]);

      setMatrixData(matrix);

      // Tạo attributeGroups ban đầu
      // const uniqueAttributes = Array.from(
      //   new Set(
      //     productVariants.flatMap((v) =>
      //       v.productAttributeRequests.map((a) => a.name)
      //     )
      //   )
      // );
      // const newAttributeGroups: AttributeGroup[] = uniqueAttributes.map(
      //   (name) => ({
      //     name,
      //     values: Array.from(
      //       new Set(
      //         productVariants.flatMap((v) =>
      //           v.productAttributeRequests
      ...
      //   })
      // );
      // console.log("newAttributeGroups: ", newAttributeGroups);
      // setAttributeGroups(() => newAttributeGroups);

      initializeAttributeGroups(productVariants);

      // Đánh dấu đã khởi tạo xong
      initialLoadRef.current = false;
    } else {
      console.log("productVariants that bai: ", productVariants);
    }
  }, [productVariants]);
...
//     const newProductVariantRequests: ProductVariantRequest[] = newMatrix.map(
//       (row) => {
//         const attributeCells = row.filter((cell) => cell.type === "attribute");
//         return {
//           productVariantId: 0,
//           sku: (row.find((cell) => cell.type === "sku")?.value as string) || "",
//           image:
//             (row.find((cell) => cell.type === "image")?.value as string) || "",
//           changeImage: false,
//           originalPrice:
//             Number(row.find((cell) => cell.type === "originalPrice")?.value) ||
//             0,
//           price: Number(row.find((cell) => cell.type === "price")?.value) || 0,
//           quantity:
//             Number(row.find((cell) => cell.type === "quantity")?.value) || 0,
//           productAttributeRequests: attributeCells.map((cell, index) => ({
//             name: attributeGroups[index]?.name || `Attribute ${index + 1}`,
...
//     setValue("productVariantRequests", newProductVariantRequests);
//   }, [attributeGroups, matrixData, setValue]);
...
// export default UpdateProductSalesInfo;
